...
void forwardInfo(char *rname, int serverfd, int fd);
...
{
	rio_t rio;
	char buf[MAXLINE], rname[MAXLINE], method[MAXLINE] = { 0 }, uri[MAXLINE] = { 0 }, protocol[MAXLINE] = { 0 };
	int n, serverfd, size;
	...
	//查找缓存
	if(!findCacheObj(buf, content, &size))
	{
	...
	//转发信息
	forwardInfo(rname, serverfd, connfd);
	Close(serverfd);
}
...
void forwardInfo(char *name, int serverfd, int fd)
{
	rio_t rio;
	char buf[MAXLINE];
	int n;
